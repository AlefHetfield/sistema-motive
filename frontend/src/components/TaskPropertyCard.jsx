...
export default function TaskPropertyCard({ property }) {
  ...
  return <div className={`flex gap-3 rounded-xl border p-3 ${unavailable ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}>
    <div className="flex h-16 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-gray-100">{cover && !imageFailed ? <img src={cover} alt="" className="h-full w-full object-cover" loading="lazy" onError={() => setImageFailed(true)} /> : <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 text-gray-400"><Building2 size={22} /><span className="mt-1 text-[8px] font-semibold uppercase">Sem foto</span></div>}</div>
