...
export default function TaskPropertyCard({ property, compact = false }) {
  ...
  return <div className={`flex min-w-0 rounded-xl border ${compact ? 'w-full max-w-md gap-2.5 p-2' : 'gap-3 p-3'} ${unavailable ? 'border-red-300 bg-red-50' : compact ? 'border-slate-100 bg-slate-50/70' : 'border-gray-200 bg-white'}`}>
    ...
    <div className="min-w-0 flex-1"><p className={`${compact ? 'truncate text-xs' : 'break-words text-sm'} font-semibold text-gray-800`}>{property.code ? `${property.code} · ` : ''}{property.title}</p><p className={`${compact ? 'mt-0.5 truncate' : 'mt-1'} text-xs text-gray-500`}>{[property.neighborhood, property.city].filter(Boolean).join(' · ') || property.address}</p>
      <p className={`${compact ? 'mt-0.5' : 'mt-1'} text-xs font-semibold text-gray-600`}>{property.price != null ? property.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : 'Valor não informado'} · {property.status}</p>
