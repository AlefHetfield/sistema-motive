...
function MobileMonthCalendar({ cursor, days, events, todayKey, loading, onCreate, onEdit }) {
  ...
  return <div className="min-w-0 bg-slate-50/50">
    <div className="grid grid-cols-3 gap-x-3 gap-y-2 border-b border-slate-100 bg-white px-3 py-2.5">
      {EVENT_LEGEND.map(item => <span key={item.label} className="inline-flex min-w-0 items-center gap-1.5 text-[10px] font-bold text-slate-500"><span className={`h-2 w-2 shrink-0 rounded-full ${item.color}`} /><span className="truncate">{item.label}</span></span>)}
    </div>
...
export default function CalendarPage() {
  ...
  return <div className="mx-auto flex min-h-full max-w-[1600px] flex-col p-3 sm:p-4 lg:p-6">
    ...
    <div className="grid min-h-0 min-w-0 flex-1 grid-cols-[minmax(0,1fr)] gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
      <section className="app-card min-w-0 overflow-hidden">
        {mobile ? <header className="border-b border-slate-100 p-3">
          <div className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-2">
            <div className="flex items-center gap-1"><Button variant="secondary" size="sm" onClick={goToToday}>Hoje</Button><button type="button" onClick={() => goToPeriod(-1)} aria-label={viewMode === 'week' ? 'Semana anterior' : 'Mês anterior'} className="flex h-9 w-8 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100"><ChevronLeft className="h-5 w-5" /></button><button type="button" onClick={() => goToPeriod(1)} aria-label={viewMode === 'week' ? 'Próxima semana' : 'Próximo mês'} className="flex h-9 w-8 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100"><ChevronRight className="h-5 w-5" /></button></div>
            <h3 className="min-w-0 truncate text-right text-base font-bold capitalize text-slate-900">{periodTitle}</h3>
          </div>
          <div className="mt-3 grid w-full min-w-0 grid-cols-3 rounded-xl bg-slate-100 p-1">
            <button type="button" onClick={() => changeViewMode('month')} className={`min-w-0 rounded-lg px-2 py-2 text-xs font-bold transition ${viewMode === 'month' ? 'bg-white text-primary shadow-sm' : 'text-slate-500'}`}>Lista</button>
            <button type="button" onClick={() => changeViewMode('calendar')} className={`min-w-0 rounded-lg px-2 py-2 text-xs font-bold transition ${viewMode === 'calendar' ? 'bg-white text-primary shadow-sm' : 'text-slate-500'}`}>Calendário</button>
            <button type="button" onClick={() => changeViewMode('week')} className={`min-w-0 rounded-lg px-2 py-2 text-xs font-bold transition ${viewMode === 'week' ? 'bg-white text-primary shadow-sm' : 'text-slate-500'}`}>Semana</button>
          </div>
        </header> : <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-4">
          ...
          <h3 className="text-center text-lg font-bold capitalize text-slate-900">{periodTitle}</h3>
          <div className="flex items-center gap-2"><span className="inline-flex rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-500">{visibleEvents.length} compromisso(s)</span><div className="inline-flex rounded-xl bg-slate-100 p-1"><button type="button" onClick={() => changeViewMode('month')} className={`rounded-lg px-3 py-1.5 text-xs font-bold transition ${viewMode === 'month' ? 'bg-white text-primary shadow-sm' : 'text-slate-500'}`}>Mês</button><button type="button" onClick={() => changeViewMode('week')} className={`rounded-lg px-3 py-1.5 text-xs font-bold transition ${viewMode === 'week' ? 'bg-white text-primary shadow-sm' : 'text-slate-500'}`}>Semana</button></div></div>
        </header>}
