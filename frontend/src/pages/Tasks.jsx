  ...
  return <div className="task-page mx-auto max-w-[1500px] p-3 sm:p-4 md:p-7">
    ...
    <div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="hidden text-xl font-bold text-slate-800 lg:block">{activeList?.name||views.find(item=>item[0]===view)?.[1]} <span className="text-sm font-normal text-gray-400">{!loading&&data?`(${data.total})`:''}</span></h3>{mobile && !loading && data && <p className="text-xs font-semibold text-gray-500">{data.total} tarefa(s)</p>}{options?.canManageAll&&view!=='day'&&<select aria-label="Filtrar responsável" value={assigneeId} className={`${inputClass} ${mobile ? 'w-full' : ''}`} onChange={e=>{setAssigneeId(e.target.value);setPage(1);setLoading(true);}}><option value="">Toda a equipe</option>{options.users.map(user=><option key={user.id} value={user.id}>{user.id===options.userId?'Minhas tarefas':user.nome}{!user.isActive?' (inativo)':''}</option>)}</select>}</div>
