...
const taskVisual = task => {
  if (task.status === 'DONE') return { border: 'border-l-emerald-400', dot: 'bg-emerald-500', label: 'Concluída' };
  const due = task.dueDate?.slice(0, 10);
  if (due && due < today()) return { border: 'border-l-red-400', dot: 'bg-red-500', label: 'Atrasada' };
  if (due === today()) return { border: 'border-l-amber-400', dot: 'bg-amber-500', label: 'Vence hoje' };
  if (task.status === 'WAITING') return { border: 'border-l-orange-300', dot: 'bg-orange-400', label: 'Aguardando retorno' };
  if (task.isPrivate) return { border: 'border-l-violet-400', dot: 'bg-violet-500', label: 'Pendente' };
  if (due) return { border: 'border-l-sky-400', dot: 'bg-sky-500', label: 'Planejada' };
  return { border: 'border-l-slate-200', dot: 'bg-slate-400', label: statusLabels[task.status] };
};
    ...
    <div className={`grid gap-5 ${compactSidebar ? 'lg:grid-cols-[72px_minmax(0,1fr)]' : 'lg:grid-cols-[230px_minmax(0,1fr)]'}`}>
      ...
      <main className="min-w-0 space-y-4">
        ...
        {loading?<p role="status" className="p-8 text-center text-sm text-gray-500">Carregando tarefas…</p>:<div className={taskLayout==='grid'?'flex flex-col gap-2 xl:grid xl:grid-cols-2 xl:items-start xl:gap-3':''}>{taskColumns.map((column,columnIndex)=><div key={columnIndex} className={taskLayout==='grid'?'contents xl:block xl:space-y-3':'space-y-2'}>{column.map(({task:originalTask,index:taskIndex}) => { const patch = optimisticChanges.get(originalTask.id); const task = { ...originalTask, ...patch }; const taskBusy = busy || Boolean(patch); const visual = taskVisual(task); return <article key={task.id} style={taskLayout==='grid'?{order:taskIndex}:undefined} aria-busy={Boolean(patch)} className={`group relative grid grid-cols-[2rem_minmax(0,1fr)] items-start gap-2 overflow-visible rounded-xl border border-l-4 border-gray-200 bg-white p-3 shadow-sm transition hover:-translate-y-px hover:border-slate-300 hover:shadow-md lg:flex lg:gap-2.5 ${visual.border}`}>
          ...
          <div className="min-w-0 flex-1 pr-9 lg:pr-0"><button type="button" disabled={taskBusy} onClick={()=>openTask(task)} className="w-full text-left"><span className={`flex items-center gap-2 break-words text-[15px] font-semibold ${task.status==='DONE'?'text-gray-400 line-through':'text-slate-800'}`}><span>{task.title}</span>{task.isNew&&<span className="shrink-0 rounded-full bg-sky-100 px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wide text-sky-700 no-underline">Nova</span>}</span>{task.notes?.trim()&&<span className="mt-1 flex items-start gap-1.5 text-xs leading-5 text-gray-500"><StickyNote size={13} className="mt-0.5 shrink-0 text-slate-400"/><span className="line-clamp-2 whitespace-pre-line text-left">{task.notes.trim()}</span></span>}<span className="mt-1.5 flex flex-wrap items-center gap-x-2.5 gap-y-1 text-xs text-gray-500"><span className="inline-flex items-center gap-1 font-medium text-slate-600"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-indigo-50 text-[9px] font-bold text-indigo-700">{task.assignee.nome.charAt(0)}</span>{task.assignee.nome}</span>{task.isPrivate&&<span className="inline-flex items-center gap-1 font-medium text-violet-600"><LockKeyhole size={12}/>Pessoal</span>}{task.client&&<span className="truncate text-slate-500">{task.client.nome}</span>}{task.dueDate&&<span className={task.dueDate.slice(0,10)<today()&&task.status!=='DONE'?'font-semibold text-red-600':'text-slate-500'}>{taskDateLabel(task.dueDate)}</span>}<span role={patch ? 'status' : undefined} className="inline-flex items-center gap-1.5 font-medium text-slate-500"><span className={`h-1.5 w-1.5 rounded-full ${visual.dot}`}/>{patch ? (patch.remove ? 'Removendo…' : 'Salvando…') : visual.label}</span>{task.steps.length>0&&<span>{task.steps.filter(step=>step.done).length}/{task.steps.length} etapas</span>}</span></button>{task.property && <div className="mt-2"><TaskPropertyCard property={task.property} compact /></div>}</div>
          ...
          <div className={`hidden h-8 flex-wrap items-center justify-end gap-1 transition lg:flex ${task.important || task.myDay?.slice(0,10)===today() ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 group-focus-within:opacity-100'}`}>{canReorder && <><button type="button" disabled={busy || optimisticChanges.size > 0 || (data.page === 1 && data.tasks[0]?.id === task.id)} aria-label={`Subir ${task.title} na fila`} onClick={() => movePublication(task, 'up')} className="rounded-lg p-1.5 text-gray-500 hover:bg-slate-100 disabled:opacity-30"><ArrowUp size={17} /></button><button type="button" disabled={busy || optimisticChanges.size > 0 || (data.page === data.pages && data.tasks.at(-1)?.id === task.id)} aria-label={`Descer ${task.title} na fila`} onClick={() => movePublication(task, 'down')} className="rounded-lg p-1.5 text-gray-500 hover:bg-slate-100 disabled:opacity-30"><ArrowDown size={17} /></button></>}<button type="button" disabled={taskBusy} aria-label={`Importância de ${task.title}`} aria-pressed={task.important} onClick={()=>changeTask(task,{important:!task.important})} className={`rounded-lg p-1.5 hover:bg-amber-50 ${task.important?'text-amber-500':'text-gray-400 hover:text-amber-500'}`}><Star size={18} fill={task.important?'currentColor':'none'}/></button><button type="button" disabled={taskBusy} aria-label={`Adicionar ${task.title} ao meu dia`} onClick={()=>changeTask(task,{myDay:task.myDay?.slice(0,10)===today()?null:today()})} className={`rounded-lg p-1.5 hover:bg-primary/5 ${task.myDay?.slice(0,10)===today()?'text-primary':'text-gray-400 hover:text-primary'}`}><Sun size={18}/></button><button type="button" disabled={taskBusy} aria-label={`Excluir ${task.title}`} onClick={()=>changeTask(task,{remove:true},true)} className="rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600"><Trash2 size={17}/></button></div>
