...
export default function Tasks() {
  ...
  return <div className="task-page mx-auto max-w-[1500px] p-3 sm:p-4 md:p-7">
    <div className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-primary">Organize sua rotina</p><h2 className="mt-1 text-2xl font-bold text-slate-900 sm:text-3xl">Cada tarefa, um próximo passo.</h2><p className="mt-2 text-sm text-gray-500">{options?.canManageAll ? 'Acompanhe suas prioridades e as tarefas da equipe.' : 'Seu espaço para acompanhar as tarefas atribuídas a você.'}</p></div><button type="button" disabled={!options} onClick={()=>setEditor({new:true})} className="flex items-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-white disabled:opacity-40"><Plus size={18}/> {view === 'social' ? 'Nova publicação' : 'Nova tarefa'}</button></div>
    {mobile && <button type="button" aria-expanded={showTaskLists} aria-controls="task-lists" onClick={() => setShowTaskLists(value => !value)} className="mb-3 flex min-h-11 w-full items-center justify-between rounded-xl border bg-white px-4 py-3 text-left text-sm font-semibold text-primary"><span>{activeList?.name || views.find(item => item[0] === view)?.[1]}</span><span>{showTaskLists ? 'Fechar' : 'Trocar lista'}</span></button>}
    <div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 space-y-4">
        ...
        {loading?<p role="status" className="p-8 text-center text-sm text-gray-500">Carregando tarefas…</p>:<div className="space-y-2">{data?.tasks.map(task=><article key={task.id} className="grid grid-cols-[2.75rem_minmax(0,1fr)] items-start gap-2 rounded-xl border border-gray-200 bg-white p-3 shadow-sm sm:flex sm:gap-3 sm:p-4">
