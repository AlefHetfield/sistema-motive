        ...
        <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
          <main className={surfaceClass}>
            ...
            <div className="p-5 sm:p-6">
              ...
              {currentStep === 3 && <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2"><SelectField label="Categoria" value={data.imovel.categoria} onChange={value => updateSection('imovel', 'categoria', value)} placeholder="Selecione" options={[{ value: '', label: 'Selecione' }, { value: 'casa', label: 'Casa' }, { value: 'apartamento', label: 'Apartamento' }, { value: 'terreno', label: 'Terreno' }]} /><Field label="Número da matrícula" value={data.imovel.matricula} onChange={event => updateSection('imovel', 'matricula', event.target.value)} /><Field label="Cartório responsável" value={data.imovel.cartorio} onChange={event => updateSection('imovel', 'cartorio', event.target.value)} /><Field label="Endereço do imóvel" value={data.imovel.endereco} onChange={event => updateSection('imovel', 'endereco', event.target.value)} /></div><label className="block"><span className={formLabelClass}>Descrição jurídica completa <span className="font-normal text-gray-400">(opcional quando os campos acima estiverem completos)</span></span><textarea value={data.imovel.descricao} onChange={event => updateSection('imovel', 'descricao', event.target.value)} rows={6} placeholder="Cole aqui a descrição conforme consta na matrícula..." className={textAreaClass} /></label><p className="flex gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />Confira a descrição diretamente na matrícula antes de gerar o contrato.</p></div>}
